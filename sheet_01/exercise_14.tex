...
\begin{enumerate}[label=(\arabic*),start=22]
  ...
  \item
    ...
    \end{align*}
    We find for~$\abs{i-j} \geq 2$ that~$\delta_{i+1,j} = \delta_{i,j+1} = 0$ and hence
    \[
      \ad(E_i)^{-a_{ji}+1}(E_j)
      =
      \ad(E_i)(E_j)
    ...
    \]
    as desired.
    If~$i = j-1$ then~$\ad(E_i)(E_j) = E_{i,j+1} = E_{i,i+2}$ and~$a_{ji} = -1$ and thus
    \begin{align*}
      \ad(E_i)^{-a_{ji}+1}(E_j)
      &=
      \ad(E_i)^2(E_j)
  ...
  \item
    This can be done by similar calculations as~(26) but can also be directly derived from~(26) by again using the matrix transpose.
\end{enumerate}
